...
@Component({
  ...
  styleUrls: ['./sowing.component.css']
})
export class SowingComponent implements OnInit {
  ...
  ngOnInit() {
    this.adminService.getAll('Sowing?take='+ this.take).subscribe((data :any) => {
  ...
  submitForm(model: any) {
    ...
    this.adminService.post('Sowing', model).subscribe((data) => {
      this.TableData = data.list;
      this.moreExist = data.exist 
      this.initializeForm();
  ...
  removeItem(Id) {
    this.adminService.delete('Sowing', Id).subscribe((data) => {
      this.TableData = data.list;
      this.moreExist = data.exist 
    }, error => {
      console.log("error occured")
